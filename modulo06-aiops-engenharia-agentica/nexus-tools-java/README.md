# Nexus Tools API em Java

Porte em Java/Spring Boot das **ferramentas** (tools/*.py, decoradas com `@tool` do CrewAI) do projeto Nexus AIOps do módulo 06 do curso — expostas como endpoints HTTP simples.

**Escopo desta porta**: apenas a lógica de negócio das 18 ferramentas foi portada. A orquestração multi-agente hierárquica do CrewAI (`core/agents.py`, `labs/*.py`, o "Nexus Manager" que delega tarefas a agentes especialistas via LLM) **não** foi replicada — não existe framework equivalente pronto em Java/Go, e reimplementá-la do zero seria um projeto de arquitetura à parte, fora do escopo de um porte mecânico. Na versão Python, essas ferramentas eram invocadas pelo LLM via function calling dentro de um `Crew`; aqui, cada uma é um endpoint HTTP independente que pode ser chamado diretamente ou integrado a qualquer orquestrador (inclusive um agent loop já portado neste repositório, como [`agent-loop-framework-go`](../../modulo04-agentes-autonomos/agent-loop-framework-go)).
...
Todas as respostas seguem o envelope `{"result": "<texto>"}` — equivalente ao retorno `String` de cada função `@tool` na versão Python.
...
- Mesmas regras de negócio: threshold de erro do canary, regras de governança OPA (região `us-east-1`, bloqueio de `t3.large`, bloqueio de ingress `0.0.0.0/0`), aprovação humana obrigatória (`GESTOR-APROVA`) para comandos Terraform destrutivos, mapa de remediações conhecidas (`OOMKilled`, `ImagePullBackOff`, `CrashLoopBackOff`).
...
- Mesma leitura de runbooks em `data/runbook_<service>.md` (o arquivo `runbook_db.md` foi copiado para `data/` deste projeto).

## O que foi adaptado

| Aspecto | Python/CrewAI | Java |
|---|---|---|
| Invocação da tool | Function calling do LLM dentro de um `Crew`/`Agent` do CrewAI | Endpoint HTTP `POST /tools/<nome>`, chamável por qualquer orquestrador |
| Execução de subprocessos | `subprocess.run(["kubectl", ...])` / `subprocess.run(["checkov", ...])` | `ProcessBuilder` |
| Geração/serialização de JSON (dashboard Grafana) | `json.dumps(dict, indent=2)` | Jackson `ObjectMapper` |
...
Cobrem os principais ramos de decisão de cada ferramenta (K8s ops/diag, security scan, observabilidade, AIOps, ChatOps, governança, policy RAG, file writer, runbook), incluindo geração/leitura real de arquivos em disco, além de um smoke test HTTP via MockMvc. Testados com sucesso em JDK 17 e JDK 25 (este projeto não usa Mockito, então não é afetado pela incompatibilidade Byte Buddy/JDK 25 encontrada em outros portes deste módulo, ex. `brag-bot-java`).
